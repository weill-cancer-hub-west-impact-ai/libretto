<script lang="ts">
  ...
  import type { ExtractionSpec, GenerationMessage } from '$lib/extraction_types';
  import type { PromptOnlySpecContent } from '$lib/extraction_types';
  ...
  let editedCombineNotes: boolean = false;
  let editedMaxChunkSize: number = 100000;
  ...
  $: if (selectedSpec) {
    let content = selectedSpec.content as PromptOnlySpecContent;
    editedSpecName = selectedSpec.name;
    editedPrompt = content.prompt;
    editedCombineNotes = content.combine_notes ?? false;
    editedMaxChunkSize = content.max_chunk_size ?? 100000;
  ...
  $: hasChanges =
    !!selectedSpec &&
    (editedSpecName !== selectedSpec.name ||
      editedPrompt !== (selectedSpec.content as PromptOnlySpecContent).prompt ||
      editedCombineNotes !==
        ((selectedSpec.content as PromptOnlySpecContent).combine_notes ?? false) ||
      editedMaxChunkSize !==
        ((selectedSpec.content as PromptOnlySpecContent).max_chunk_size ?? 100000) ||
      !areObjectsEqual(
        examplePatientIds,
        (selectedSpec.content as PromptOnlySpecContent).example_patient_ids ?? []
      ) ||
    ...
    try {
      const response = await authenticatedFetch(`/api/projects/${$projectID}/specs/${newID}`, {
        ...
        body: JSON.stringify({
          ...
          content: {
            prompt: editedPrompt,
            combine_notes: editedCombineNotes,
            max_chunk_size: editedMaxChunkSize,
  ...
  function _generationBody(prompt?: string, answers?: string[], evaluate: boolean = false): any {
    ...
    let requestBody: any = {
      spec_draft: {
        example_patient_ids: examplePatientIds,
        prompt: editedPrompt,
        combine_notes: editedCombineNotes,
        max_chunk_size: editedMaxChunkSize
      },
  ...
  async function handleGenerateSubmit(prompt?: string, qaAnswers?: string[]) {
    if (!selectedSpec || (!prompt && !qaAnswers)) return;
    ...
    try {
      const response = await authenticatedFetch(
        `/api/projects/${$projectID}/specs/${selectedSpec.id}/generate_prompt_only`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(_generationBody(prompt, qaAnswers, examplePatientIds.length > 0))
        }
      );
  ...
  <div class="flex h-full flex-auto flex-col">
    ...
    <div class="px-4">
      <SpecGenerationView
        ...
        {generating}
        historyType="spec_generation_prompt_only"
    ...
    <div class="min-h-0 flex-auto overflow-auto px-4 py-4">
      ...
      <div class="space-y-4 rounded-lg border-2 border-stone-300/50 p-4 shadow-lg">
        ...
        <div>
          <div class="mb-1 text-sm font-bold uppercase">
            ...
            <span class="ml-2 font-normal text-stone-500 normal-case">
              Instructions to run against each note.
            </span>
          </div>
          <CodeMirror
            class="max-h-96 w-full overflow-auto rounded-md border border-stone-300"
        ...
        <div class="flex items-center gap-2">
          <input
            id="combine-notes"
            type="checkbox"
            bind:checked={editedCombineNotes}
            class="h-4 w-4 rounded border-stone-300 text-blue-600 focus:ring-blue-500"
          />
          <label for="combine-notes" class="text-sm font-bold uppercase">
            Combine Notes
            <span class="ml-2 font-normal text-stone-500 normal-case">
              {#if editedCombineNotes}
                Notes will be combined within the maximum length (specified below) for processing.
              {:else}
                Notes will be processed independently.
              {/if}
            </span>
          </label>
        </div>
        ...
        {#if editedCombineNotes}
          <!-- Max Chunk Size -->
          <div>
            <label for="max-chunk-size" class="mb-1 block text-sm font-bold uppercase">
              Max Character Length
            </label>
            <input
              id="max-chunk-size"
              type="number"
              bind:value={editedMaxChunkSize}
              min="1000"
              step="1000"
              class="w-48 rounded-md border border-stone-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none"
              placeholder="100000"
            />
          </div>
        {/if}
      ...
      <div class="my-2 text-xs text-stone-500">
        This specification was created as <strong>prompt-only</strong>: it runs a single prompt over
        all notes for each patient.
      </div>
    </div>
  </div>
{/if}
